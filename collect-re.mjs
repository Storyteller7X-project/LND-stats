 ...
 * ----------------------------
 * Sbírá zápasy formátu "Region Expressions" hraného přes NeatQueue Discord ladder.
 * Roster (Riot ID) bere z NeatQueue API, decky + výsledky z Riot LoR match API
 * (Bo3 challenge lobby zápasy). Filtr: započítá jen zápasy, kde jsou OBA hráči
 * v rosteru -> spolehlivě čistě formátové zápasy.
 *
 * Výstup: data/re-stats.json (stejná struktura jako standard -> stejný dashboard).
 * Stav ve Firebase pod nodem `re-meta` (sdílený projekt s LND-stats).
 *
 * ENV (GitHub Actions Secrets):
 *   RIOT_API_KEY, FIREBASE_SECRET, NEATQUEUE_TOKEN
 */
import fs from 'node:fs/promises';
import path from 'node:path';

const CONFIG_PATH = path.join(process.cwd(), 'config.json');
const STATS_PATH  = path.join(process.cwd(), 'data', 're-stats.json');

const DEFAULT_CONFIG = {
  firebaseUrl: '',
  neatQueueBase: 'https://api.neatqueue.com',
  neatQueueServerId: '1131890486303535175',
  stateNode: 're-meta',
  clusters: ['americas', 'europe', 'asia'],
  maxWindowDays: 60,            // formát je řídký -> drž delší okno
  patchStart: '2026-06-03',
  maxRequestsPerRun: 120,       // konzervativně (sdílí Riot klíč se standard collectorem)
  maxMatchesPerPuuid: 15,
  maxMatchDetailsPerRun: 40,
  champRefreshHours: 168,
  dataDragonSets: ['set1','set2','set3','set4','set5','set6','set6cde','set7','set7b','set8','set9','set10','set11','set12']
};
// ---------------------------------------------------------------------------
...
async function loadState(dbUrl, secret, node) {
  try {
    const res = await fetch(`${dbUrl}/${node}.json?auth=${secret}`);
    if (res.ok) { const d = await res.json(); return d ? decodeKeys(d) : {}; }
    console.warn(`[Firebase] load: ${res.status}`);
  } catch (e) { console.warn(`[Firebase] load selhal: ${e.message}`); }
  return {};
}
async function saveState(dbUrl, secret, node, state) {
  const res = await fetch(`${dbUrl}/${node}.json?auth=${secret}`, {
...
// ---------------------------------------------------------------------------
// NeatQueue: roster (Riot ID / IGN)
// ---------------------------------------------------------------------------
async function nqGet(cfg, token, pathSeg) {
  const res = await fetch(`${cfg.neatQueueBase}${pathSeg}`, { headers: { Authorization: token } });
  if (!res.ok) { console.warn(`[NeatQueue] ${res.status} ${pathSeg}`); return null; }
  return res.json();
}
const looksLikeRiotId = (s) => typeof s === 'string' && /#/.test(s) && s.length <= 40 && s.length >= 3;
// projde libovolný tvar odpovědi a vytáhne řetězce "Jméno#Tag" pod IGN-like klíči
function extractIgns(data) {
  const igns = new Set();
  const walk = (o) => {
    if (!o) return;
    ...
    if (typeof o === 'object') {
      for (const [k, v] of Object.entries(o)) {
        if (typeof v === 'string' && /ign|riot|ingame|in.?game|account|^name$|gamename/i.test(k) && looksLikeRiotId(v)) igns.add(v);
        else walk(v);
      }
    }
  };
...
const splitRiotId = (ign) => { const i = ign.lastIndexOf('#'); return i < 0 ? [null, null] : [ign.slice(0, i).trim(), ign.slice(i + 1).trim()]; };

async function resolvePuuid(ign, limiter, apiKey, state) {
  if (state.puuidCache[ign]) return state.puuidCache[ign];
  const [name, tag] = splitRiotId(ign);
  if (!name || !tag) return null;
  const acc = await riotGet('americas',
    `/riot/account/v1/accounts/by-riot-id/${encodeURIComponent(name)}/${encodeURIComponent(tag)}`, limiter, apiKey);
  const p = acc?.puuid;
  if (p) state.puuidCache[ign] = p;
  return p;
}
// LoR match-v1 je per-cluster; neznáme region hráče -> zkusíme clustery a cache-neme
async function matchIdsAnyCluster(puuid, limiter, apiKey, state, clusters) {
  const cached = state.puuidCluster[puuid];
  const order = cached ? [cached, ...clusters.filter(c => c !== cached)] : clusters;
  for (const c of order) {
    const ids = await riotGet(c, `/lor/match/v1/matches/by-puuid/${puuid}/ids`, limiter, apiKey);
    if (Array.isArray(ids) && ids.length) { state.puuidCluster[puuid] = c; return { cluster: c, ids }; }
  }
  return { cluster: order[0], ids: [] };
}
...
async function main() {
  ...
  const nqToken = process.env.NEATQUEUE_TOKEN;
  if (!apiKey) throw new Error('Chybí RIOT_API_KEY');
  if (!fbSecret) throw new Error('Chybí FIREBASE_SECRET');
  if (!nqToken) throw new Error('Chybí NEATQUEUE_TOKEN');

  let cfg = DEFAULT_CONFIG;
  try { cfg = { ...DEFAULT_CONFIG, ...JSON.parse(await fs.readFile(CONFIG_PATH, 'utf8')) }; }
  ...
  const NODE = cfg.stateNode;

  const limiter = new RateLimiter(cfg.maxRequestsPerRun, [
    { ms: 1000, max: 18 }, { ms: 120000, max: 95 }
  ]);

  const state = await loadState(dbUrl, fbSecret, NODE);
  state.processed ??= [];
  state.days ??= {};
  state.champMap ??= {};
  state.cardRegions ??= {};
  ...
  // champ mapa + regiony karet (jako standard)
  ...
  if (champStale || Object.keys(state.champMap).length === 0 || Object.keys(state.cardRegions || {}).length === 0) {
    try {
      ...
      if (Object.keys(fresh.champMap).length > 0) {
        state.champMap = fresh.champMap; state.cardRegions = fresh.cardRegions; state.champFetchedAt = Date.now();
      }
    } catch (e) { console.warn(`[DataDragon] ${e.message}`); }
  }
  const champMap = state.champMap, cardRegions = state.cardRegions || {};

  // 1) roster z NeatQueue
  const raw = await nqGet(cfg, nqToken, `/api/v1/playerstats/${cfg.neatQueueServerId}`);
  console.log('[NeatQueue] odpověď typ:', Array.isArray(raw) ? 'array' : typeof raw,
    '| náhled:', JSON.stringify(raw).slice(0, 600));
  const igns = extractIgns(raw);
  console.log(`[roster] nalezeno IGN: ${igns.length}`, JSON.stringify(igns.slice(0, 10)));

  // 2) IGN -> PUUID
  const rosterPuuids = new Set();
  for (const ign of igns) {
    try { const p = await resolvePuuid(ign, limiter, apiKey, state); if (p) rosterPuuids.add(p); }
  ...
  const gameTypeTally = {}; const infoSamples = [];
  let md = 0, recorded = 0;
  try {
    for (const puuid of rosterPuuids) {
      if (md >= cfg.maxMatchDetailsPerRun) break;
      const { cluster, ids } = await matchIdsAnyCluster(puuid, limiter, apiKey, state, cfg.clusters);
      for (const matchId of (ids || []).slice(0, cfg.maxMatchesPerPuuid)) {
        ...
        processed.add(matchId); md++;
        if (!m?.info) continue;
        const gt = m.info.game_type || m.info.type || '?';
        ...
        for (const [lab, dc] of [[la, pa.deck_code], [lb2, pb.deck_code]]) {
          const comp = (state.comp[lab] ??= { n: 0, c: {} }); comp.n++;
          for (const { code, count } of decodeDeckCounts(dc)) { const e = (comp.c[code] ??= [0, 0]); e[0]++; e[1] += count; }
        }
        const fmt = String(m.info.game_format ?? m.info.format ?? 'eternal');
        const bucket = bucketFor(state.days, dk, cluster, fmt);
        for (const [lab, won] of [[la, aWon], [lb2, !aWon]]) {
          bucket.deckGames[lab] ??= { games: 0, wins: 0 }; bucket.deckGames[lab].games++; if (won) bucket.deckGames[lab].wins++;
        }
        if (la !== lb2) {
          const kAB = `${la}@@${lb2}`, kBA = `${lb2}@@${la}`;
          bucket.matchups[kAB] ??= { games: 0, wins: 0 }; bucket.matchups[kBA] ??= { games: 0, wins: 0 };
    ...
    if (e.message === 'RUN_CAP' || e.message === 'RATE_HALT') console.log(`[stop] ${e.message} — ukládám progress`);
  ...
  for (const [lab, c] of Object.entries(state.comp)) {
    ...
    if (codes.length > 80) { const keep = codes.sort((a, b) => c.c[b][0] - c.c[a][0]).slice(0, 80); const nc = {}; for (const k of keep) nc[k] = c.c[k]; c.c = nc; }
  }
  await saveState(dbUrl, fbSecret, NODE, state);

  const stats = {
    generatedAt: new Date().toISOString(),
    patchStart: cfg.patchStart,
    regions: cfg.clusters,
    windows: [
  ...
  console.log(`[hotovo] requestů: ${limiter.used}, IGN: ${igns.length}, PUUID: ${rosterPuuids.size}, zápasů zaznamenáno: ${recorded}, detailů: ${md}, dnů: ${Object.keys(state.days).length}`);
  console.log('[game_type DISTINCT]', JSON.stringify(gameTypeTally));
  console.log('[info SAMPLE]', JSON.stringify(infoSamples).slice(0, 800));
}
